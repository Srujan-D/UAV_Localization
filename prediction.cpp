...
// TODO: Add white gausian noise 

Matrix3d Prediction::R_q(State *s){
...
State* Prediction::f(State* prev_state, State* current_state, State* imu_measured_state){
    ...
    Vector3d a_imu;
    a_imu << imu_measured_state->a.linear.x, imu_measured_state->a.linear.y, imu_measured_state->a.linear.z + State::g;
    ...
    new_state->v.linear.z = prev_state->v.linear.z + (d_vel(2,0) + State::g)* d_time;
...
Matrix10d Prediction::F(State* prev_state, State* current_state){
...
Matrix10d Prediction::PredictedCovariance(State* prev_state, State* current_state){
    noiseQk <<  0.5, 0, 0, 0, 0, 0, 0, 0, 0, 0,
        ...
        Matrix10d jacobi = F(prev_state,new_state);
        Matrix10d jacobiTransposed= jacobi.transpose();
